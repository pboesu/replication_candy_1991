...
Additional simulations (Appendix) showed that the coefficient of variation for estimates derived from a wide range of initial values was c. 0.6\% and c. 1.7\% for the logit and cloglog models, respectively, whenenver the model converged succesfully (Table~\ref{tab:tabS2}).
This means the observed differences between the published parameter estimates and those obtained using R \verb+optim+ are on the order of approximately 2 standard deviations of the expected variation caused by the choice of initial values.
The uncertainty about initial values may therefore explain the differences in the parameter estimates between the different methods. 
