...
budworm_sratio_cloglog <- brm(stage ~ ddeg, data = budworm_individuals, family = sratio(link = 'cloglog'), inits = "0", chains = 2, cores = 2)
...
budworm_cumulative_cloglog <- brm(stage ~ ddeg, data = budworm_individuals, family = brms::cumulative(link = 'cloglog'), inits = initfun, chains = 2, cores = 2)#param estimates more or less identical to sratio model, as demonstrated by Laara and Matthews
budworm_cumulative_cloglog_cs <- brm(stage ~ cs(ddeg), data = budworm_individuals, family = brms::cumulative(link = 'cloglog'), inits = initfun, chains = 1, cores = 2)#not possible to fit
